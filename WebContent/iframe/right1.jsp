...
<%
	...
	pageCount=ResourceService.getPageCount(pageSize);
...
if(request.getAttribute("rows")!=null){		
	...
	al=ResourceService.getResourcesByPage(pageNow, pageSize);
}
%>
	<tr><th>院系</th><th>专业</th><th>课程</th><th>教师</th><th>主题</th><th>说明</th><th>链接内容</th><th>链接密码</th><th>学号</th><th>姓名</th></tr>
<%
		for(Resource r:al){
			
%>
	<tr><td><%=r.getDepartment() %></td><td><%=r.getProfession() %></td><td><%=r.getCourse() %></td><td><%=r.getTeacher() %></td><td><%=r.getTopic() %></td><td><%=r.getDescription() %></td><td><a href="<%=r.getLocation() %>" target="_blank" >下载</a></td><td><%=r.getLockey() %></td><td><%=r.getStuno() %></td><td><%=r.getStuname() %></td></tr>
...
<a href="iframe/right1.jsp?myPageNow=<%=pageNow%>">上一页</a>
	
<%
}else {
%>

<a href="iframe/right1.jsp?myPageNow=<%=pageNow-1%>">上一页</a>
<% 
}
...
%>
<a href="iframe/right1.jsp?myPageNow=<%=i%>"> < <%=i%> > </a>
<%
}
%>
...
%>
<a href='iframe/right1.jsp?myPageNow=<%=pageNow%>'>下一页</a>
<%
}else {
%>
<a href='iframe/right1.jsp?myPageNow=<%=pageNow+1%>'>下一页</a>
<%
}
%>
</div>
